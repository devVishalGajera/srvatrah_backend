...
route.get('/packages', async (req,res,next) =>{
    ...
    const filterList = packageList.map(data =>{
        ...
        return ({duration:data.packageDuration,
...
route.get('/package/details/:id', async (req,res,next) =>{
    ...
    const packageInfo = await packageCollection.findById(packageID);
    const response = {packageDuration:packageInfo.packageDuration,
                    _id:packageInfo._id,
...
route.get('/package/iti/:id', async (req,res,next) =>{
    ...
    if (!req.cookies.room_guest){
        ...
        for (let car of packageInfo.availableVehicle){
            const carAvailable = await vehicleCollection.find({seatLimit:car.seatLimit,vehicleType:car.vehicleType,vehicleCategory:car.vehicleCategory,city:'Cochin'})
    ...
    const result = {itinerary:await Promise.all( packageInfo.itinerary.map(async(data) =>{
        return  (
        {title:data.title,
        dayCount: data.dayCount,
        place:data.place,
        activities: await Promise.all(data.activities.map( async(act)=>{
            if (act.objType ==="car"){
                let carResult = {...sortedCar[0], ...{sequence:act.sequence, objType:act.objType}}
                return (
                  carResult
        )
            }
            else if (act.objType ==="hotel"){
                const hotelAvailable = await hotelCollection.find({city:data.place})
    ...
    for (let data of packageInfo.itinerary){
        for (let act of data.activities){
            if (act.objType ==="hotel"){
                ...
                hotelList = hotelAvailable.sort((a,b) =>a.rooms[0].occupancy1-b.rooms[0].occupancy1)
        ...
        hotelResult.push({
            ...
            rooms:hotel.rooms.map((vac) =>{
                ...
                for (let traveller of guestLists){
                    ...
                    if (traveller.cb){
                        travellerPrice = travellerPrice + vac.child.childWithBedPrice * traveller.cb
                    }
                    if (traveller.cwb){
                        travellerPrice = travellerPrice + vac.child.childWithoutBedPrice * traveller.cwb
                    }
                    console.log(traveller)
                }
                console.log(travellerPrice)

                return ({roomType:vac.roomType,
...
route.post('/package',upload.fields([{name:'file',maxCount:1},{name:'subfile',maxCount:10}]), async (req,res,next) =>{
    ...
    const themeImg ={filename:req.files.file[0].filename,path:'http://127.0.0.1:3232/'+req.files.file[0].path.replace('public/',''),mimetype:req.files.file[0].mimetype}
