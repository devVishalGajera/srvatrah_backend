...
const { vehicleCollection, hotelCollection, Category } = require('../models/inventries')
...
route.post('/vehicle', upload.single('file'), async (req, res, next) => {
  ...
  res.status(200).json({ data: [vehicleType, brandName, modelName, inventry, seatLimit, active] })
})
...
route.get('/vehicle', async (req, res, next) => {
  ...
  if ((!vehicleType) && (!brandName)) {
    ...
    const getBrandName = await vehicleCollection.find({ vehicleType: vehicleType },)
