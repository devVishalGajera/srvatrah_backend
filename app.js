...
// app.use(cors({ origin: 'https://v1.sarvatrah.com',credentials:true}));
...
app.post('/token/:type', (req, res) => {
    ...
    jwt.sign(req.body, secretKey, (err, token) => {
      if (err) {
        ...
        res.setHeader('Set-Cookie', cookie.serialize('roomInfo', token, {
        ...
        jwt.sign(req.body, secretKey, (err, token) => {
            if (err) {
              ...
              res.setHeader('Set-Cookie', cookie.serialize('itineraryData', token, {
